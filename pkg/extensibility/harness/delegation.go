...
// FromEnv returns the fixed agent-harness CLI configuration for the active
// cm2labs profile. It does not read endpoint or credential values from a task.
func FromEnv() (HarnessCLI, bool) {
	binary, profileDir, routes := os.Getenv("CM2LABS_HARNESS_BINARY"), os.Getenv("CM2LABS_RUNTIME_PROFILE_DIR"), os.Getenv("CM2LABS_DELEGATION_ROUTES")
	if binary == "" || profileDir == "" || routes == "" {
		return HarnessCLI{}, false
	}
	return HarnessCLI{Binary: binary, ProfileDir: profileDir, Routes: routes, PicoBaseURL: os.Getenv("CM2LABS_PICO_BASE_URL"), HermesBaseURL: os.Getenv("CM2LABS_HERMES_BASE_URL")}, true
}
